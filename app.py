...
# =============================================================================
    ...
    try:
        ...
        # Teacher activity summary (aggregate only - no individual tracking)
    ...
    st.markdown(f"""
        <div style="background: {NAVY}; color: white; padding: 16px; border-radius: 4px; margin-bottom: 20px;">
            <h3 style="color: {GOLD}; margin: 0;">SB 1288 Compliance Status</h3>
        </div>
    """, unsafe_allow_html=True)
    ...
    if init_summary:
        ...
        with resp_cols[0]:
            st.metric("Confirmed", init_summary.get('confirmed', 0))
        with resp_cols[1]:
            st.metric("Modified", init_summary.get('modified', 0))
        with resp_cols[2]:
            st.metric("Challenged", init_summary.get('challenged', 0))
    ...
    if obs_summary and obs_summary.get('total_observations', 0) > 0:
        ...
        with col1:
            st.metric("Students Observed", int(obs_summary.get('unique_students', 0)))
        with col2:
            st.metric("Observation Days", int(obs_summary.get('observation_days', 0)))
        with col3:
            st.metric("Total Observations", int(obs_summary.get('total_observations', 0)))
        with col4:
            st.metric("Active Teachers", int(obs_summary.get('teachers_observing', 0)))
        with col5:
            concerns = int(obs_summary.get('total_concerns', 0))
            st.metric("Concern Flags", concerns, delta="review" if concerns > 0 else None, delta_color="inverse")

        # Participation pattern analysis
        st.markdown("---")
        st.markdown("**Participation Patterns (District-Wide)**")

        total_present = obs_summary.get('total_present', 0) or 1  # Avoid division by zero
        oral_rate = (obs_summary.get('total_oral', 0) / total_present) * 100
        written_rate = (obs_summary.get('total_written', 0) / total_present) * 100
        engaged_rate = (obs_summary.get('total_engaged', 0) / total_present) * 100

        pattern_cols = st.columns(4)
        with pattern_cols[0]:
            st.metric("Attendance Rate", f"{(obs_summary.get('total_present', 0) / obs_summary.get('total_observations', 1)) * 100:.0f}%")
        with pattern_cols[1]:
        ...
        # Pattern alert
        if oral_rate > written_rate + 15:
            st.warning(f"⚠️ **District Pattern Alert:** Oral participation ({oral_rate:.0f}%) exceeds written output ({written_rate:.0f}%) by >15% — consistent with Type 4 indicators across classrooms.")
    ...
    if intervention_summary:
        responding = intervention_summary.get('responding', 0) or 0
        not_responding = intervention_summary.get('not_responding', 0) or 0
        ...
        if total_interv > 0:
            ...
            with col1:
                ...
                st.markdown(f"""
                    <div class="stat-card">
                        <div class="number" style="color: {color};">{effectiveness_rate:.0f}%</div>
                        <div class="label">Effectiveness Rate</div>
                    </div>
                """, unsafe_allow_html=True)

            with col2:
                st.metric("Responding", int(responding))
            with col3:
                st.metric("Not Responding", int(not_responding))
            with col4:
                vera_conf = intervention_summary.get('vera_confirmed', 0) or 0
                st.metric("VERA Confirmed", int(vera_conf))
